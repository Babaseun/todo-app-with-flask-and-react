...
def register():
     ...
     if user_exists:
          ...
          try:
               new_user = User(id=_ID,name=data['name'],email=data['email'],password=hashedPassword)
...
def todos():
     if request.method == "POST":
          try:
               data = request.get_json()
               _ID = str(uuid.uuid4())
               new_todo = Todos(id=_ID,todo=data['todo'],owner_id=g.user['id'],created_at=data['created_at'],completed=data['completed'])
